...
package com.deflatedpickle.somftcraft.client.render.entity.feature
...
abstract class StuckObjectsFeatureRenderer<T : LivingEntity, M : EntityModel<T>>(entityRenderer: LivingEntityRenderer<T, M>) :
    ...
    override fun render(
        ...
        for (n in 0 until getObjectCount(livingEntity)) {
            ...
            val partsList = recurseModelPart(model.`somftcraft$getPieces`(), mutableListOf())
    ...
    private fun recurseModelPart(list: List<ModelPart>, possibleParts: MutableList<ModelPart>): List<ModelPart> {
        for (i in list) {
