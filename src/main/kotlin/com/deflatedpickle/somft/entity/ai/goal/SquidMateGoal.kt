...
class SquidMateGoal(
    ...
    override fun canStart(): Boolean {
        return if (!(animal as Breedable).`somft$isInLove`()) {
            false
        } else {
            mate = findMate()
            mate != null
    ...
    override fun shouldContinue(): Boolean {
        return mate!!.isAlive && (mate as Breedable).`somft$isInLove`() && timer < 60
    }
    ...
    private fun findMate(): SquidEntity? {
        ...
        for (animalEntity2 in list) {
            if ((animal as Breedable).`somft$canBreedWith`(animalEntity2) && animal.squaredDistanceTo(animalEntity2) < d) {
    ...
    private fun breed() {
        (animal as Breedable).`somft$breed`(world as ServerWorld, mate)
    }
}
